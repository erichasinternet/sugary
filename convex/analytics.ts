...
export const getSignupTrends = query({
  ...
  handler: async (ctx) => {
    ...
    const companySubscribers = subscribers.filter(s => 
      featureIds.includes(s.featureId)
    );
    
    // Group by day for the last 30 days
    const thirtyDaysAgo = Date.now() - (30 * 24 * 60 * 60 * 1000);
    const recentSubscribers = companySubscribers.filter(s => 
      s.subscribedAt >= thirtyDaysAgo
    );
...
export const getTopFeatures = query({
  ...
  handler: async (ctx) => {
    ...
    // Get features with subscriber counts
    const features = await ctx.db
      .query("features")
      .withIndex("by_company", (q) => q.eq("companyId", company._id))
      .collect();
